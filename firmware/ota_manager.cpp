...
bool OtaManager::initialize() noexcept {
    ...
    // Keep the HTTP read timeout comfortably below the 30 s task watchdog
    // window so a blocking GET can never race the WDT.
...
bool OtaManager::connectServer() noexcept {
    ...
    // Below the 30 s WDT window so a blocking GET cannot race the watchdog.
    m_http.setTimeout(20000);
...
void OtaManager::rebootDevice() noexcept {
    ...
    systemManager.requestRestart("ota_apply", false);
}
