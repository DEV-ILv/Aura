...
private:
    ...
    unsigned long m_chunkRemaining = 0;   ///< bytes left in the current chunk
    String m_chunkLine;                    ///< partial chunk-size line across ticks
    bool m_bodyComplete;
    ...
    // Reused across streaming chunks to avoid a fresh JsonDocument pool
    // allocation/free per SSE chunk.
    JsonDocument m_streamParseDoc;

    // Response size limiting
    bool m_responseTruncated;
    ...
    static constexpr uint32_t kMaxResponseTextChars = 32768U;
    static constexpr uint32_t kMaxBodyBufferBytes = 65536U;
