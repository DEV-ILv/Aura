...
namespace {
  ...
  // ---------------------------------------------------------------------------
  // Atomic settings blob (P0-04)
  //
  // saveAll() previously wrote ~32 individual NVS keys; a power loss mid-save
  // left a mixed old/new configuration. NVS guarantees a single putBytes() is
  // atomic (one key = one flash commit), so the whole Settings struct is now
  // serialized into ONE versioned, CRC-protected key. load() prefers the blob
  // and falls back to the legacy per-key layout (migration) when the blob is
  // absent or corrupt, so existing devices upgrade transparently.
  // ---------------------------------------------------------------------------
  constexpr const char* SETTINGS_BLOB_KEY = "blob";
  constexpr uint32_t SETTINGS_BLOB_MAGIC = 0x41555231;  // "AUR1"
  constexpr uint8_t  SETTINGS_BLOB_VERSION = 1;
  constexpr size_t   SETTINGS_BLOB_MAX = 160;

  // Fixed-layout payload size (LE byte order, no padding/ABI dependence).
  constexpr size_t PAYLOAD_STRINGS = 32 + 32 + 32 + 8;   // deviceName, hostname, wakeWord, language
  constexpr size_t PAYLOAD_SCALARS = 32;                 // all scalar fields below
  constexpr size_t PAYLOAD_SIZE = PAYLOAD_STRINGS + PAYLOAD_SCALARS;

  uint32_t settingsCrc32(const uint8_t* data, size_t len) noexcept {
  ...
  // Serialize a Settings struct into the blob payload (fixed layout).
  size_t serializeSettings(const Settings& s, uint8_t* out) noexcept {
    uint8_t* p = out;
    auto putU8  = [&p](uint8_t v) { *p++ = v; };
    auto putU16 = [&p](uint16_t v) { *p++ = static_cast<uint8_t>(v & 0xFF); *p++ = static_cast<uint8_t>(v >> 8); };
    auto putI32 = [&p](int32_t v) {
      uint32_t u = static_cast<uint32_t>(v);
      for (int i = 0; i < 4; ++i) { *p++ = static_cast<uint8_t>(u & 0xFF); u >>= 8; }
    };
    auto putStr = [&p](const char* src, size_t cap) {
      size_t n = 0;
      while (n < cap && src[n] != '\0') { *p++ = static_cast<uint8_t>(src[n]); ++n; }
      while (n < cap) { *p++ = 0; ++n; }
    };

    putStr(s.deviceName, 32);
    putStr(s.hostname, 32);
    putStr(s.wakeWord, 32);
    putStr(s.language, 8);

    putU8(s.wifiAutoConnect ? 1 : 0);
    putU8(s.screenBrightness);
    putU16(s.screenTimeout);
    putU8(s.ledBrightness);
    putU8(s.ledEnabled ? 1 : 0);
    putU8(s.discoBrightness);
    putU8(s.volume);
    putU8(s.microphoneGain);
    putU8(s.wakeWordEnabled ? 1 : 0);
    putU8(s.continuousListeningEnabled ? 1 : 0);
    putU8(s.silentMode ? 1 : 0);
    putU8(s.privacyMode ? 1 : 0);
    putU8(s.outputToOled ? 1 : 0);
    putU8(s.outputToSpeaker ? 1 : 0);
    putU8(s.outputToCompanion ? 1 : 0);
    putU8(s.nightModeEnabled ? 1 : 0);
    putU8(s.nightModeStartHour);
    putU8(s.nightModeStartMinute);
    putU8(s.nightModeEndHour);
    putU8(s.nightModeEndMinute);
    putU16(s.autoSleepTimeoutMin);
    putI32(s.timezone);
    putU8(s.use24HourClock ? 1 : 0);
    putU8(s.reminderEnabled ? 1 : 0);
    putU8(s.debugLogging ? 1 : 0);
    putU8(s.otaEnabled ? 1 : 0);
    putU8(s.autoUpdate ? 1 : 0);

    return static_cast<size_t>(p - out);
  }

  // Deserialize a blob payload back into a Settings struct.
  bool deserializeSettings(Settings& s, const uint8_t* in, size_t len) noexcept {
    if (len < PAYLOAD_SIZE) return false;
    const uint8_t* p = in;
    auto getU8  = [&p]() { return *p++; };
    auto getU16 = [&p]() { uint16_t v = static_cast<uint16_t>(p[0]) | (static_cast<uint16_t>(p[1]) << 8); p += 2; return v; };
    auto getI32 = [&p]() {
      uint32_t u = 0;
    ...
    auto getStr = [&p](char* dst, size_t cap) {
      size_t n = 0;
      while (n < cap && *p != 0) { dst[n] = static_cast<char>(*p++); ++n; }
      dst[n] = '\0';
      p += (cap - n);
    };

    getStr(s.deviceName, 32);
    getStr(s.hostname, 32);
    getStr(s.wakeWord, 32);
    getStr(s.language, 8);

    s.wifiAutoConnect = getU8() != 0;
    s.screenBrightness = getU8();
    s.screenTimeout = getU16();
    s.ledBrightness = getU8();
    s.ledEnabled = getU8() != 0;
    s.discoBrightness = getU8();
    s.volume = getU8();
    s.microphoneGain = getU8();
    s.wakeWordEnabled = getU8() != 0;
    s.continuousListeningEnabled = getU8() != 0;
    s.silentMode = getU8() != 0;
    s.privacyMode = getU8() != 0;
    s.outputToOled = getU8() != 0;
    s.outputToSpeaker = getU8() != 0;
    s.outputToCompanion = getU8() != 0;
    s.nightModeEnabled = getU8() != 0;
    s.nightModeStartHour = getU8();
    s.nightModeStartMinute = getU8();
    s.nightModeEndHour = getU8();
    s.nightModeEndMinute = getU8();
    s.autoSleepTimeoutMin = getU16();
    s.timezone = getI32();
    s.use24HourClock = getU8() != 0;
    s.reminderEnabled = getU8() != 0;
    s.debugLogging = getU8() != 0;
    s.otaEnabled = getU8() != 0;
    s.autoUpdate = getU8() != 0;

    return p - in == static_cast<ptrdiff_t>(len);
  }

  // Read and validate the atomic blob. Returns true and fills `out` only when
  // magic, version and CRC all match and the payload fully deserializes.
  bool readSettingsBlob(Preferences& prefs, Settings& out) noexcept {
    uint8_t buf[SETTINGS_BLOB_MAX];
    const size_t n = prefs.getBytes(SETTINGS_BLOB_KEY, buf, sizeof(buf));
    if (n < 9 + PAYLOAD_SIZE) return false;
    ...
    if (version != SETTINGS_BLOB_VERSION) return false;
    ...
    const uint32_t computedCrc = settingsCrc32(buf + 9, PAYLOAD_SIZE);
    if (storedCrc != computedCrc) return false;

    Settings tmp;
    if (!deserializeSettings(tmp, buf + 9, n - 9)) return false;
    out = tmp;
    return true;
  }

  // Write the Settings struct as one atomic, versioned, CRC-protected blob.
  bool writeSettingsBlob(Preferences& prefs, const Settings& s) noexcept {
    uint8_t buf[SETTINGS_BLOB_MAX];
    buf[0] = static_cast<uint8_t>(SETTINGS_BLOB_MAGIC & 0xFF);
    buf[1] = static_cast<uint8_t>((SETTINGS_BLOB_MAGIC >> 8) & 0xFF);
    buf[2] = static_cast<uint8_t>((SETTINGS_BLOB_MAGIC >> 16) & 0xFF);
    buf[3] = static_cast<uint8_t>((SETTINGS_BLOB_MAGIC >> 24) & 0xFF);
    buf[4] = SETTINGS_BLOB_VERSION;
    const size_t payloadLen = serializeSettings(s, buf + 9);
    const uint32_t crc = settingsCrc32(buf + 9, payloadLen);
    buf[5] = static_cast<uint8_t>(crc & 0xFF);
    buf[6] = static_cast<uint8_t>((crc >> 8) & 0xFF);
    buf[7] = static_cast<uint8_t>((crc >> 16) & 0xFF);
    buf[8] = static_cast<uint8_t>((crc >> 24) & 0xFF);
    return prefs.putBytes(SETTINGS_BLOB_KEY, buf, 9 + payloadLen) == 9 + payloadLen;
  }
...
bool SettingsManager::load() noexcept {
  ...
  // P0-04: prefer the atomic versioned blob. If it is present and valid, it is
  // the authoritative source (a single NVS write can never be partially torn).
  if (readSettingsBlob(m_preferences, m_settings)) {
    endPreferences();
    if (!validate()) {
    ...
    Logger::debug("SettingsManager", "Settings loaded from atomic blob");
    return true;
  }
  ...
  if (storedVersion != SETTINGS_VERSION && storedVersion != 0) {
    ...
    if (!migrate(storedVersion)) {
      ...
      endPreferences();
      loadDefaults();
      // Persist defaults + current version stamp so the recovery is durable
      // across reboots (never wipe the whole NVS namespace automatically).
      saveAll();
      return false;
    }
  }
  ...
  // Stamp the current version so upgrades/migrations persist across reboots
  // (and a fresh/legacy store becomes durable current-version state). saveAll()
  // also writes the atomic P0-04 blob, so a legacy per-key store is upgraded
  // to the blob on first boot even when the version already matches.
  saveAll();
...
bool SettingsManager::saveAll() noexcept {
  ...
  // P0-04: write the atomic versioned blob FIRST. A single NVS putBytes() is
  // one flash commit, so the whole Settings struct either lands or does not.
  // The per-key writes below remain for downgrade compatibility and as the
  // migration fallback; load() treats the blob as authoritative.
  ok &= writeSettingsBlob(m_preferences, m_settings);

  // Save version
  ok &= m_preferences.putUShort(VERSION_KEY, SETTINGS_VERSION);

  // Save device settings
  ok &= m_preferences.putString(DEVICE_NAME_KEY, m_settings.deviceName);
  ok &= m_preferences.putString(HOSTNAME_KEY, m_settings.hostname);

  // Save WiFi settings
  ok &= m_preferences.putBool(WIFI_AUTO_CONNECT_KEY, m_settings.wifiAutoConnect);

  // Save display settings
  ok &= m_preferences.putUChar(SCREEN_BRIGHTNESS_KEY, m_settings.screenBrightness);
  ok &= m_preferences.putUShort(SCREEN_TIMEOUT_KEY, m_settings.screenTimeout);

  // Save LED settings
  ok &= m_preferences.putUChar(LED_BRIGHTNESS_KEY, m_settings.ledBrightness);
  ok &= m_preferences.putBool(LED_ENABLED_KEY, m_settings.ledEnabled);
  ok &= m_preferences.putUChar(DISCO_BRIGHTNESS_KEY, m_settings.discoBrightness);

  // Save audio settings
  ok &= m_preferences.putUChar(VOLUME_KEY, m_settings.volume);
  ok &= m_preferences.putUChar(MIC_GAIN_KEY, m_settings.microphoneGain);

  // Save wake word settings
  ok &= m_preferences.putBool(WAKE_WORD_ENABLED_KEY, m_settings.wakeWordEnabled);
  ok &= m_preferences.putString(WAKE_WORD_KEY, m_settings.wakeWord);

  // Save push-to-talk settings
  ok &= m_preferences.putBool(CONTINUOUS_LISTENING_KEY, m_settings.continuousListeningEnabled);

  // Save interaction refinement settings
  ok &= m_preferences.putBool(SILENT_MODE_KEY, m_settings.silentMode);
  ok &= m_preferences.putBool(PRIVACY_MODE_KEY, m_settings.privacyMode);

  // Save output routing settings
  ok &= m_preferences.putBool(OUTPUT_OLED_KEY, m_settings.outputToOled);
  ok &= m_preferences.putBool(OUTPUT_SPEAKER_KEY, m_settings.outputToSpeaker);
  ok &= m_preferences.putBool(OUTPUT_COMPANION_KEY, m_settings.outputToCompanion);
  ok &= m_preferences.putBool(NIGHT_MODE_ENABLED_KEY, m_settings.nightModeEnabled);
  ok &= m_preferences.putUChar(NIGHT_START_HOUR_KEY, m_settings.nightModeStartHour);
  ok &= m_preferences.putUChar(NIGHT_START_MIN_KEY, m_settings.nightModeStartMinute);
  ok &= m_preferences.putUChar(NIGHT_END_HOUR_KEY, m_settings.nightModeEndHour);
  ok &= m_preferences.putUChar(NIGHT_END_MIN_KEY, m_settings.nightModeEndMinute);
  ok &= m_preferences.putUShort(AUTO_SLEEP_KEY, m_settings.autoSleepTimeoutMin);

  // Save localization settings
  ok &= m_preferences.putString(LANGUAGE_KEY, m_settings.language);
  ok &= m_preferences.putInt(TIMEZONE_KEY, m_settings.timezone);
  ok &= m_preferences.putBool(CLOCK_24H_KEY, m_settings.use24HourClock);

  // Save reminder settings
  ok &= m_preferences.putBool(REMINDER_ENABLED_KEY, m_settings.reminderEnabled);

  // Save debug/system settings
  ok &= m_preferences.putBool(DEBUG_LOGGING_KEY, m_settings.debugLogging);
  ok &= m_preferences.putBool(OTA_ENABLED_KEY, m_settings.otaEnabled);
  ok &= m_preferences.putBool(AUTO_UPDATE_KEY, m_settings.autoUpdate);

  endPreferences();

  if (!ok) {
    // A partial NVS failure must not silently claim success: report it so the
    // caller (restart/shutdown flush paths) knows persistence is incomplete.
    Logger::error("SettingsManager", "One or more settings failed to persist");
    return false;
  }
