...
public:
     ...
     */
    [[nodiscard]] bool isSPIFFSMounted() noexcept;

    /**
     * @brief Check if SD card is mounted
     * @return true if SD card is mounted, false otherwise
     */
    [[nodiscard]] bool isSDMounted() noexcept;
    /**
...
private:
    // Bounded, atomic-friendly log append with rotation. Writes at most
    // kLogRecordMaxBytes in a single write and renames to .old at the cap so a
    // torn append (power loss) can only ever damage one line.
    StorageStatus appendLogRecord(const char* path, const char* data, size_t dataSize) noexcept;

    // SD hot-plug: throttled presence probe + bounded remount. Called from
    // update() so the loop task detects card removal/reinsertion at runtime.
    void checkSDHotPlug() noexcept;
    bool isSDModulePresent() noexcept;
    ...
    StorageStatus recoverSdOrphanedTmps() noexcept;
    ...
    // Storage mutex lock/unlock (protects SD/SPIFFS from mount task + loop task)
    bool lockStorage(TickType_t timeout = portMAX_DELAY) noexcept;
    void unlockStorage() noexcept;
...
static constexpr const char* LOG_DIR          = "/logs";
    ...
    SemaphoreHandle_t m_storageMutex;  ///< Protects SD/SPIFFS ops from mount task + loop task

    // Log append bounds (single-write record + rotation cap)
    static constexpr size_t kLogRecordMaxBytes{512U};
    static constexpr size_t kMaxLogFileSize{64U * 1024U};

    // SD hot-plug detection/remount throttling
    unsigned long m_lastSdProbeMs{0};        ///< Last SD presence probe timestamp
    unsigned long m_lastSdRemountMs{0};      ///< Last SD remount attempt timestamp
    uint8_t m_sdProbeFailures{0};            ///< Consecutive failed probes
    bool m_sdRemountFailLogged{false};       ///< Logged the current remount-fail run
    static constexpr unsigned long kSdProbeIntervalMs{5000UL};
    static constexpr unsigned long kSdRemountIntervalMs{30000UL};
    static constexpr uint8_t kSdProbeFailThreshold{3};
};
