...
public:
    ...
    /**
     * @brief Re-run Wire + display initialization after a detected I2C failure.
     * @return true if re-initialization succeeded, false otherwise
     * @note Throttled by the caller (HealthManager); never called per-frame.
     */
    [[nodiscard]] bool reinitialize() noexcept;
...
void reset() noexcept;
    ...
    /**
     * @brief Check whether the OLED actually answers on the I2C bus right now.
     * @return true if the panel ACKs an I2C probe, false otherwise
     * @note Cheap, non-destructive Wire probe used by the periodic health check
     *       to detect a display that died at runtime (isInitialized() alone
     *       cannot see a hardware fault). Never call per-frame; the caller
     *       (HealthManager/SystemManager) throttles it.
     */
...
private:
    ...
    Adafruit_SH1106G m_display{OLED_WIDTH, OLED_HEIGHT, &Wire, OLED_RESET};   ///< SH1106 OLED display object
