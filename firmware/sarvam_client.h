...
//   * Speech-to-Text  -> SarvamClient::startTranscription()      (buffered multipart)
//                        SarvamClient::startTranscriptionStream() (chunked streaming)
...
public:
    ...
    /**
     * Begin an async STT upload that streams PCM with Transfer-Encoding: chunked.
     * The multipart head is sent first; PCM is fed via streamPcm(); the tail and
     * terminating chunk are queued by endTranscriptionStream(). No Content-Length
     * and no full-utterance buffer are required.
     */
    void startTranscriptionStream(const char* head, size_t headLen,
                                  const char* tail, size_t tailLen) noexcept;
    /** Feed a PCM chunk into the streaming upload. False if the send ring is full. */
    bool streamPcm(const uint8_t* pcm, size_t len) noexcept;
    /** Queue the multipart tail + terminating chunk, then move to RECEIVING. */
    void endTranscriptionStream() noexcept;
    /** True while a streaming STT upload request is active. */
    bool sttStreamActive() const noexcept;
...
private:
    ...
    void sendStreamChunk() noexcept;
    void readIncoming() noexcept;
    void ringPut(uint8_t b) noexcept;
    ...
    // --- streaming STT (chunked transfer-encoding)
    static constexpr size_t kStreamRingCap = 4096;
    bool m_streaming{false};                 ///< streaming upload in progress
    bool m_streamTailQueued{false};          ///< tail + terminating chunk queued
    uint8_t* m_streamRing{nullptr};          ///< ring of framed body bytes awaiting send
    size_t m_streamRingHead{0};
    size_t m_streamRingTail{0};
    size_t m_streamRingCount{0};
    uint8_t* m_streamHead{nullptr};          ///< multipart head (chunked body part)
    size_t m_streamHeadLen{0};
    uint8_t* m_streamTail{nullptr};          ///< multipart tail + codec field
    size_t m_streamTailLen{0};
    uint8_t* m_streamScratch{nullptr};       ///< contiguous slice of ring for socket write
