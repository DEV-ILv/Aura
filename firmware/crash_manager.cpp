...
void CrashManager::checkCrashOnBoot() noexcept {
    ...
    if (crashReset) {
        ...
        // Software restart: distinguish deliberate restarts from unexplained
        // or recovery-triggered restarts by reading the persisted reason.
        Preferences prefs;
        String rstReason;
        if (prefs.begin(CRASH_COUNTER_NVS_NAMESPACE, true)) {
            rstReason = prefs.getString(CRASH_RESTART_REASON_KEY, "");
            prefs.end();
        }

        // Deliberate restart reasons that should NOT increment the boot counter.
        const char* deliberateReasons[] = {
            "user_restart",
            "web_restart",
            "command_restart",
            "ota_apply",
        ...
        for (const char* dr : deliberateReasons) {
            if (rstReason == dr) {
                isDeliberate = true;
                break;
            }
        ...
        if (isDeliberate) {
            Logger::info(kLogCategory, "Deliberate software restart: %s",
                rstReason.isEmpty() ? "(no reason)" : rstReason.c_str());
            m_bootCounter = 0;
            writeBootCounter(0);
        } else {
            // Unexplained SW restart or health_reboot_* — treat as crash loop candidate.
            logCrash("Unexplained software restart on boot", "SystemStartup",
                     "rst_reason: " + rstReason);
            Logger::warning(kLogCategory, "Previous boot was software restart (%s) - reason: %s",
                getResetReasonString(reason), rstReason.isEmpty() ? "(none)" : rstReason.c_str());
        ...
        // Clean boot reset (POWERON, DEEP_SLEEP, EXT, etc.) — reset counter.
        m_bootCounter = 0;
        writeBootCounter(0);
    }
}

bool CrashManager::wasRecoveryRestart() noexcept {
    Preferences prefs;
    String rstReason;
    if (prefs.begin(CRASH_COUNTER_NVS_NAMESPACE, true)) {
        rstReason = prefs.getString(CRASH_RESTART_REASON_KEY, "");
        ...
        // NVS unreadable: cannot confirm a clean boot. Fail closed so the
        // boot-loop counter is preserved and safe mode can still trigger.
        return true;
    }
    return rstReason.startsWith("health_reboot_");
}
...
void CrashManager::writeBootCounter(int count) noexcept {
    ...
    if (!m_prefs.putInt(CRASH_COUNTER_KEY, count)) {
        Logger::error(kLogCategory, "Failed to persist boot counter %d", count);
    }
    m_prefs.end();
}
