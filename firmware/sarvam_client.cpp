...
SarvamClient::~SarvamClient() noexcept {
    cancel();
    if (m_streamRing) { free(m_streamRing); m_streamRing = nullptr; }
    if (m_streamHead) { free(m_streamHead); m_streamHead = nullptr; }
    if (m_streamTail) { free(m_streamTail); m_streamTail = nullptr; }
    if (m_streamScratch) { free(m_streamScratch); m_streamScratch = nullptr; }
}
...
void SarvamClient::cancel() noexcept {
    ...
    m_audioDone = false;
    m_streaming = false;
    m_streamTailQueued = false;
    m_streamRingHead = 0;
    m_streamRingTail = 0;
    m_streamRingCount = 0;
    m_streamHeadLen = 0;
    m_streamTailLen = 0;
    // Note: we do NOT free the allocated buffers here; they are reused across sessions
    // to avoid repeated malloc/free overhead. They are freed in the destructor.
}
...
void SarvamClient::startTranscriptionStream(const char* head, size_t headLen,
                                            const char* tail, size_t tailLen) noexcept {
    cancel();
    ...
    m_header += m_apiKey;
    m_header += "\r\nContent-Type: multipart/form-data; boundary=BOUNDARY101\r\n";
    m_header += "Transfer-Encoding: chunked\r\n";
    m_header += "Connection: close\r\n\r\n";

    // Allocate streaming buffers on first use to avoid BSS bloat at boot
    if (!m_streamRing) m_streamRing = (uint8_t*)malloc(kStreamRingCap);
    if (!m_streamHead) m_streamHead = (uint8_t*)malloc(256);
    if (!m_streamTail) m_streamTail = (uint8_t*)malloc(256);
    if (!m_streamScratch) m_streamScratch = (uint8_t*)malloc(2048);
    if (!m_streamRing || !m_streamHead || !m_streamTail || !m_streamScratch) {
        LOG_ERROR("SarvamClient", "Failed to allocate streaming buffers");
        m_httpErr = SarvamHttpError::NETWORK;
        m_stt.clear();
        m_stt.ok = false;
        m_finalized = true;
    ...
    // Keep the multipart head/tail alive for the duration of this request.
    m_streamHeadLen = (head && headLen <= 256) ? headLen : 0;
    if (m_streamHeadLen > 0) memcpy(m_streamHead, head, headLen);
    m_streamTailLen = (tail && tailLen <= 256) ? tailLen : 0;
    if (m_streamTailLen > 0) memcpy(m_streamTail, tail, tailLen);

    m_phase = Phase::CONNECTING;

    // Queue the multipart head as the first chunked frame so it streams as soon
    // as the TLS connection is up.
    if (m_streamHeadLen > 0) {
        char hex[16];
        snprintf(hex, sizeof(hex), "%zx\r\n", m_streamHeadLen);
        const size_t hexLen = strlen(hex);
        if (m_streamRingCount + hexLen + m_streamHeadLen + 2 <= kStreamRingCap) {
            for (size_t i = 0; i < hexLen; ++i) ringPut((uint8_t)hex[i]);
            for (size_t i = 0; i < m_streamHeadLen; ++i) ringPut(m_streamHead[i]);
            ringPut('\r');
            ringPut('\n');
        }
    }
...
bool SarvamClient::sttStreamActive() const noexcept {
    return m_streaming && m_kind == Kind::STT && m_phase != Phase::IDLE;
}

bool SarvamClient::streamPcm(const uint8_t* pcm, size_t len) noexcept {
    if (!m_streaming || m_streamTailQueued) return false;
    if (len == 0) return true;
    if (len > 512) len = 512;

    // Frame the chunk: "<hexlen>\r\n<pcm>\r\n"
    char hex[16];
    snprintf(hex, sizeof(hex), "%zx\r\n", len);
    const size_t hexLen = strlen(hex);
    const size_t need = hexLen + len + 2;
    if (m_streamRingCount + need > kStreamRingCap) return false;
    for (size_t i = 0; i < hexLen; ++i) ringPut((uint8_t)hex[i]);
    for (size_t i = 0; i < len; ++i)      ringPut(pcm[i]);
    ringPut('\r');
    ringPut('\n');
    return true;
}

void SarvamClient::endTranscriptionStream() noexcept {
    if (!m_streaming) return;
    m_streamTailQueued = true;
}

void SarvamClient::ringPut(uint8_t b) noexcept {
    m_streamRing[m_streamRingHead] = b;
    m_streamRingHead = (m_streamRingHead + 1) % kStreamRingCap;
    ++m_streamRingCount;
}
...
void SarvamClient::run() noexcept {
    ...
    // so that large TTS downloads are not cut short. A stalled server that stops
    // sending mid-body is caught by the receive stall watchdog below (m_lastActivity
    // only advances on actual byte progress).
    ...
    // RECEIVING stall watchdog: no byte progress for longer than the operation
    // timeout means the peer stalled mid-body. TTS streams keep feeding chunks,
    // so active downloads are never cut short.
...
void SarvamClient::sendStreamChunk() noexcept {
    // HTTP header first.
    if (m_headerSent < m_header.length()) {
        const size_t remain = m_header.length() - m_headerSent;
        const size_t want = (remain > 1024) ? 1024 : remain;
    ...
    // The head chunk was queued into the ring by startTranscriptionStream();
    // drain whatever framed bytes are pending (head, PCM chunks, tail).
    if (m_streamRingCount > 0) {
        // Copy a contiguous slice of the ring into a scratch buffer.
        const size_t take = (m_streamRingCount > 2048)
                                ? 2048
                                : m_streamRingCount;
        for (size_t i = 0; i < take; ++i) {
            m_streamScratch[i] = m_streamRing[(m_streamRingTail + i) % kStreamRingCap];
        }
        const size_t n = m_http.write(m_streamScratch, take);
        if (n == 0) {
            if (!m_http.isConnected()) abort(SarvamHttpError::NETWORK, true);
            return;
        }
        m_streamRingTail = (m_streamRingTail + n) % kStreamRingCap;
        m_streamRingCount -= n;
        m_lastActivity = millis();
    }

    // Enqueue the tail chunk (codec field + closing boundary) once the PCM is
    // fully drained, then the terminating chunk "0\r\n\r\n".
    if (m_streamRingCount == 0 && m_streamTailQueued && m_streamTailLen > 0) {
        char hex[16];
        snprintf(hex, sizeof(hex), "%zx\r\n", m_streamTailLen);
        const size_t hexLen = strlen(hex);
        const size_t need = hexLen + m_streamTailLen + 2 + 5;
        if (m_streamRingCount + need <= kStreamRingCap) {
            for (size_t i = 0; i < hexLen; ++i) ringPut((uint8_t)hex[i]);
            for (size_t i = 0; i < m_streamTailLen; ++i) ringPut(m_streamTail[i]);
            ringPut('\r');
            ringPut('\n');
            ringPut('0');
            ringPut('\r');
    ...
    if (n == 0) {
        if (!m_http.isConnected() && m_http.available() == 0) {
            if (m_contentLength >= 0 && (long)m_bodyReceived >= m_contentLength) {
                ...
                finalizeResponse();
            } else if (m_kind == Kind::STT && m_contentLength < 0 && !m_recvBuffer.isEmpty()) {
                // Close-delimited STT response (no Content-Length): any received
                // body with a successful status is treated as complete.
                finalizeResponse();
            } else {
                abort(SarvamHttpError::NETWORK, true);
            }
...
void SarvamClient::abort(SarvamHttpError err, bool transient) noexcept {
    m_http.close();
    // Streaming uploads cannot be retried: audio already written to the socket
    // cannot be replayed, so a mid-stream failure must surface immediately.
    if (transient && !m_streaming && m_attempt < m_cfg.retryCount) {
        ++m_attempt;
        m_httpErr = err;
