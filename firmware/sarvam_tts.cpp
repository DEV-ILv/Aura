...
void SarvamTextToSpeech::update() noexcept {
    ...
    if (m_srvActive) {
        ...
        // Waiting for the ring + speaker to flush. isPlaying() stays true once
        // startPlayback() is called, so completion is driven by time: once the
        // ring is empty give the DMA a short settle, then stop and finish.
        unsigned long now = millis();
        if (m_ringCount == 0) {
            if (m_audioPushedBytes == 0 || (now - m_lastPushMs >= kDrainSettleMs)) {
                if (audioManager.isPlaying()) audioManager.stopPlayback();
                finishItem(true, TTSError::NONE);
            }
        } else if (now - m_drainStartMs >= kDrainTimeoutMs) {
            // Bounded recovery: a wedged speaker must not hang the pipeline.
            LOG_WARN("SarvamTTS", "Drain timeout (%lu ms) - force finishing",
                     static_cast<unsigned long>(kDrainTimeoutMs));
            if (audioManager.isPlaying()) audioManager.stopPlayback();
            m_ringHead = m_ringTail = m_ringCount = 0;
            finishItem(m_audioPushedBytes > 0, TTSError::NONE);
        }
    }
...
void SarvamTextToSpeech::drainPlayback() noexcept {
    ...
    while (m_ringCount > 0 && guard < 4) {
        ...
        if (rd == 0) break;
        // Without a speaker channel (AURA_HW_SPEAKER_PRESENT=0) the ring can
        // never be handed to the DMA; consume it so the drain completes instead
        // of hanging until the drain timeout.
        if (!audioManager.hasSpeaker()) {
            m_audioPushedBytes += rd;
            ringAdvance(rd);
            m_lastPushMs = millis();
            continue;
        }
...
void SarvamTextToSpeech::clearError() noexcept {
    m_error = TTSError::NONE;
}
...
void SarvamTextToSpeech::ringPut(const uint8_t* data, size_t len) noexcept {
    ...
    while (len > 0) {
        ...
        // Bound the write by actual free space (kRingCap - count), not by the
        // distance to the end of the backing array: when the ring is wrapped
        // (head < tail) the tail-side bytes are live data and must be preserved.
        const size_t freeSpace = kRingCap - m_ringCount;
        const size_t n = (len < freeSpace) ? len : freeSpace;
        // Write in up to two segments so we never memcpy past the array end.
        const size_t spaceTail = kRingCap - m_ringHead;
        const size_t first = (n < spaceTail) ? n : spaceTail;
        memcpy(m_ring + m_ringHead, data, first);
        if (n > first) {
            memcpy(m_ring, data + first, n - first);
        }
