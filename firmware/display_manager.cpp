...
bool DisplayManager::initialize() noexcept {
    ...
    if (!m_display.begin(OLED_ADDRESS, true)) {
        LOG_ERROR("DisplayManager", "SH1106 initialization failed");
        return false;
    }

    m_display.clearDisplay();
    m_display.display();   // push a blank frame now so the panel doesn't show
                           // random power-on GDDRAM during the long boot phase
...
bool DisplayManager::reinitialize() noexcept {
    LOG_WARN("DisplayManager", "Re-initializing display (I2C recovery)");
    m_initialized = false;
...
bool DisplayManager::isResponsive() noexcept {
    if (!m_initialized) {
        return false;
    }
...
void DisplayManager::setInverted(bool inverted) noexcept {
    if (!m_initialized) return;
    m_inverted = inverted;
    m_display.invertDisplay(inverted);
