...
void ConversationManager::run() noexcept {
    ...
    switch (m_currentState) {
        ...
        case ConversationState::ERROR:
            ...
            } else if (now - m_stateStartTime >= kErrorRecoveryMs) {
                // Bounded recovery: a latched submodule error (STT/AI/TTS) can
                // otherwise trap the pipeline in ERROR forever. Clear the error
                // latches so checkSubmoduleErrors() releases and we return to
                // the idle face; the failed subsystem recovers independently.
                LOG_WARN("ConversationManager",
                         "ERROR stuck %lu ms - clearing submodule error latches",
                         static_cast<unsigned long>(now - m_stateStartTime));
...
void ConversationManager::handleListening() noexcept {
    ...
    if (!speechToText.isBusy()) {
        ...
        m_result.sttLatencyMs = sttResult.durationMs;
...
void ConversationManager::handleTranscribing() noexcept {
    ...
    if (!geminiClient.isBusy()) {
        ...
        m_result.llmLatencyMs = geminiResult.latencyMs;
...
void ConversationManager::handleSpeaking() noexcept {
    ...
    // Barge-in: if the user starts speaking while AURA is talking, stop the
    // speech and go straight back to listening. Requires several consecutive
    // above-threshold mic probes so a single blip does not cut us off.
    if (m_bargeInEnabled && !m_silentMode && audioManager.isInitialized()) {
        const unsigned long now = millis();
        const float voiceEnergy = audioManager.getAudioEnergy();
        const float bargeThreshold =
            static_cast<float>(audioManager.getNoiseFloor()) +
            static_cast<float>(audioManager.getNoiseThreshold());
        if (voiceEnergy > bargeThreshold) {
            if (m_bargeInStartMs == 0) {
                m_bargeInStartMs = now;
            }
            if ((now - m_bargeInStartMs) >= kBargeInRequireMs) {
                LOG_INFO("ConversationManager", "Barge-in: interrupting speech (energy %.0f)",
                         static_cast<double>(voiceEnergy));
                textToSpeech.stop();
                if (audioManager.isPlaying()) audioManager.cancelPlayback();
                m_ttsTriggered = false;
                m_bargeInStartMs = 0;
                changeState(ConversationState::LISTENING);
                return;
    ...
    if (!textToSpeech.isBusy()) {
        m_ttsTriggered = false;
        m_result.ttsLatencyMs = textToSpeech.getResponse().latencyMs;
...
void ConversationManager::extractEntitiesFromConversation() noexcept {
    ...
    if (idx >= 0) {
        skipLen = 11; // length of "my name is "
    } else {
        idx = lower.indexOf("i'm ");
        if (idx >= 0) {
            skipLen = 4; // length of "i'm "
        } else {
            idx = lower.indexOf("call me ");
            if (idx >= 0) {
                skipLen = 8; // length of "call me "
            }
        }
    }
    if (idx >= 0) {
        String name = text.substring(idx + skipLen);
...
void ConversationManager::handleCompletion() noexcept {
    ...
    LOG_INFO("ConversationManager",
        "Voice diag: stt=%lu ms llm=%lu ms tts=%lu ms total=%lu ms heap_free=%u B",
        (unsigned long)m_result.sttLatencyMs,
        (unsigned long)m_result.llmLatencyMs,
        (unsigned long)m_result.ttsLatencyMs,
        (unsigned long)m_result.latencyMs,
...
void ConversationManager::handleTimeout() noexcept {
    ...
    if (m_currentState != ConversationState::IDLE && m_currentState != ConversationState::COMPLETED && m_currentState != ConversationState::ERROR && m_currentState != ConversationState::PAUSED) {
        if (now - m_stateStartTime >= kStateTimeoutMs) {
            ...
            if (m_currentState == ConversationState::LISTENING) {
                // A LISTENING turn stuck on continuous noise must release the
                // mic and abort STT here; the ERROR->IDLE transition that
                // follows does not cancel a live STT session on its own.
                speechToText.cancelRecognition();
                if (m_micActive) stopMic();
            }
            setError(ConversationError::TIMEOUT);
