...
String MemoryManager::exportJson() const noexcept {
    // Content-aware reserve + heap floor guard (same pattern as the
    // analytics/briefing/KG serializers): building the full JSON must not
    // start when the heap cannot safely hold it.
    size_t totalLen = 32;
    for (const auto& e : m_entries) {
        totalLen += e.id.length() + e.key.length() + e.value.length() +
                    e.tags.length() + e.source.length() + e.summary.length() +
                    e.contextName.length() + 180; // fixed JSON overhead per entry
    }
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping exportJson: low heap (%u < %u + %u)",
            ESP.getFreeHeap(), totalLen, kHeapSlack);
        return String();
    }

    String json;
    json.reserve(totalLen);
    json += "[";
...
bool MemoryManager::save() noexcept {
    ...
    if (json.isEmpty()) {
        LOG_WARNING(kLogCategory, "Save skipped: exportJson returned empty (low heap)");
        return false;
    }
...
void MemoryManager::evictIfNeeded() noexcept {
    ...
    // Hard cap: if the store is still at/over the limit after the normal
    // eviction pass (e.g. every entry is persistent/pinned/high-priority),
    // force-evict the single lowest-priority entry so m_entries can never
    // grow unboundedly and exhaust heap.
    if (evicted == 0 && m_entries.size() >= m_maxEntries && !m_entries.empty()) {
        ...
        LOG_WARN(kLogCategory, "Hard-cap evicting [%s] %s (prio=%u)",
            categoryToString(victim->category), victim->key.c_str(),
            victim->priority);
...
void MemoryManager::saveRevisions() noexcept {
    ...
    size_t totalLen = 32;
    for (const auto& r : m_revisions) {
        totalLen += r.id.length() + r.memoryId.length() +
                    r.previousValue.length() + r.tags.length() + 70;
    }
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping saveRevisions: low heap (%u < %u + %u)",
            ESP.getFreeHeap(), totalLen, kHeapSlack);
        return;
    }
...
void MemoryManager::saveConversations() noexcept {
    ...
    size_t totalLen = 32;
    for (const auto& c : m_conversations) {
        totalLen += c.id.length() + c.title.length() + c.summary.length() +
                    c.importantMessages.length() + 100;
    }
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping saveConversations: low heap (%u < %u + %u)",
            ESP.getFreeHeap(), totalLen, kHeapSlack);
        return;
    }
