...
#include <Arduino.h>
#include "config.h"
// Keep the WebSockets library client-slot count in sync with WS_MAX_CLIENTS.
// The library default is 5 but this firmware tracks per-client auth state in
// m_wsAuth[WS_MAX_CLIENTS]; an unguarded slot index could write past the array.
#ifndef WEBSOCKETS_SERVER_CLIENT_MAX
#define WEBSOCKETS_SERVER_CLIENT_MAX WS_MAX_CLIENTS
#endif
#include <WebServer.h>
...
private:
    ...
    void webSocketBroadcast(const String& json) noexcept;
    ...
    void handleApiMicDiagnose() noexcept;

    // SD diagnostics
    void handleApiSdDiagnostics() noexcept;

    // Storage browser (companion SdFile contract: GET /api/storage, upload,
    // delete, download). Backed by real StorageManager data - no faked files.
    void handleApiStorageList() noexcept;
    void handleApiStorageDelete() noexcept;
    void handleApiStorageDownload() noexcept;
    void handleApiStorageUpload() noexcept;
    void handleApiStorageUploadChunk() noexcept;
    ...
    void sendToAuthenticatedClients(const String& json) noexcept;
    ...
    void recordWsAuthFailure(uint8_t num) noexcept;
    bool isWsAuthLockedOut(uint8_t num) noexcept;
    void resetWsAuthTracker(uint8_t num) noexcept;
    String wsAuthKey(uint8_t num) const noexcept;
    ...
    /// Heap-allocated at runtime in start() (never during static
    /// initialization, where the ESP32 heap is not guaranteed usable).
    ...
    /// Storage browser upload state (streamed in chunks by the WebServer).
    /// We accumulate into an in-memory buffer sized by the caller's chunk
    /// writes; bounds are enforced in the upload handler.
    String m_storageUploadName;
    String m_storageUploadBuffer;
    bool m_storageUploadActive{false};
    bool m_storageUploadDone{false};
    static constexpr size_t kStorageUploadMaxBytes{512 * 1024};
    ...
    unsigned long m_wsLastSeen[WS_MAX_CLIENTS];

    // Reused dashboard JSON buffer to avoid a fresh ~2KB heap allocation on
    // every broadcast (~2x/sec while a client is connected).
    String m_wsDashboardJson;
    ...
    // Per-IP WebSocket auth rate limiting (mirrors the HTTP login tracker but
    // keyed by WS client slot/IP, bounded like the HTTP map).
    std::map<String, LoginTracker> m_wsAuthTrackers;
    static constexpr uint8_t kWsMaxAuthAttempts{5};
    static constexpr unsigned long kWsAuthLockoutDurationMs{30000UL};
    static constexpr size_t kWsMaxTrackedIps{16};
};
