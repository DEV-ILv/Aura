...
// One-shot task creation guards: never schedule a second restart/factory/OTA
// task while one is already pending (a duplicate ESP.restart() task is a
// pointless FreeRTOS task leak and races the shutdown sequence).
static bool g_restartTaskPending{false};
static bool g_factoryResetTaskPending{false};
static bool g_otaRestartTaskPending{false};
...
bool WebPortal::start() noexcept
{
    ...
    // The WebSocket server is allocated at runtime (never during C++ static
    // initialization, where the ESP32 heap is not guaranteed usable and the
    // library's unguarded _server dereference in begin() would crash with a
    // LoadProhibited fault). Construct it only after system initialization is
    // complete, then check for allocation failure before use.
    if (m_webSocket == nullptr)
    {
        m_webSocket = new WebSocketsServer(WS_PORT);
    }

    if (m_webSocket == nullptr)
    {
        Logger::error("WebPortal", "Failed to allocate WebSocket server; aborting web server start");
        m_server.stop();
        return false;
    }
    ...
    // Enable the library's per-client heartbeat: it pings each connected
    // client every WS_PING_INTERVAL_MS, and if a client fails to answer a pong
    // within 10 s it is disconnected after 2 consecutive timeouts. This reaps
    // half-open sockets (the app's WS reconnect storms leave stale slots) so
    // later broadcast writes never block on a dead client for the library's
    // full 5 s TCP write timeout and trip the 30 s task watchdog.
    m_webSocket->enableHeartbeat(WS_PING_INTERVAL_MS, 10000UL, 2);
...
void WebPortal::handleClient() noexcept
{
    ...
    // Lightweight HTTP latency diagnostic: any handler that blocks the loop
    // for a long stretch (e.g. a synchronous Wi-Fi scan) would surface here.
    // This is what lets us prove the async scan keeps the portal responsive.
    const unsigned long start = millis();
    m_server.handleClient();
    const unsigned long elapsed = millis() - start;
    if (elapsed > 100) {
        Logger::warning("WebPortal", "HTTP request blocked loop for %lu ms", elapsed);
    }
}
...
bool WebPortal::registerApiRoutes() noexcept
{
    ...
    static const ApiRoute kRoutes[] = {
        ...
        {"/api/mic/diagnose", HTTP_GET, &WebPortal::handleApiMicDiagnose},
        ...
        {"/api/storage", HTTP_GET, &WebPortal::handleApiStorageList},
        {"/api/storage/delete", HTTP_POST, &WebPortal::handleApiStorageDelete},
        {"/api/storage/download", HTTP_GET, &WebPortal::handleApiStorageDownload},
    ...
    // Dedicated upload route with an upload handler. Must be registered BEFORE
    // the /api/* glob so the WebServer routes multipart chunks to it (a glob
    // handler registered via the 3-arg on() has no upload handler and silently
    // drops the file body).
    m_server.on("/api/storage/upload", HTTP_POST,
        [this]() { handleApiStorageUpload(); },
        [this]() { handleApiStorageUploadChunk(); });
...
void WebPortal::restartDevice() noexcept
{
    ...
    if (g_restartTaskPending) return;
    g_restartTaskPending = true;
...
void WebPortal::factoryReset() noexcept
{
    ...
    if (g_factoryResetTaskPending) return;
    g_factoryResetTaskPending = true;
...
void WebPortal::handleSaveWifi() noexcept
{
    ...
    // Provisioning sequence with 5-second non-blocking wait
    wifiManager.startWaitingToConnect(ssid.c_str(), password.c_str());
    Logger::info("WebPortal", "Wi-Fi configuration saved via form, waiting 5s before connect");
    sendSuccess("Wi-Fi configuration saved. Device will connect in 5 seconds...");
}
...
void WebPortal::handleOTA() noexcept
{
    ...
    else if (m_server.method() == HTTP_POST)
    {
        ...
        else if (upload.status == UPLOAD_FILE_WRITE)
        {
            if (m_otaInProgress)
            {
                esp_task_wdt_reset();  // keep the loop-task WDT alive during large uploads
                if (m_otaHashing) {
        ...
        else if (upload.status == UPLOAD_FILE_END)
        {
            ...
            if (m_otaInProgress && m_otaHashing) {
                ...
                if (!sig.isEmpty()) {
                    ...
                    Logger::error("WebPortal",
                        "Web OTA uploaded WITHOUT a signature - rejecting "
                        "(signed firmware required)");
                    Update.abort();
                    m_otaInProgress = false;
                    securityManager.LogAudit(AuditEventType::OTA_STARTED, "webportal",
                                             "Web OTA rejected: missing signature", false);
                    sendError("Firmware signature is required", 400);
                    return;
                }
            }
            ...
            if (m_otaInProgress && signatureOk && Update.end(true))
            {
                ...
                if (!g_otaRestartTaskPending) {
                    g_otaRestartTaskPending = true;
...
void WebPortal::handleWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) noexcept {
    // num is guaranteed to be < WEBSOCKETS_SERVER_CLIENT_MAX by the library,
    // which we now pin to WS_MAX_CLIENTS, but guard defensively anyway so a
    // future library/config mismatch can never index past m_wsAuth.
    if (num >= WS_MAX_CLIENTS) {
        Logger::error("WebPortal", "WS event with out-of-range client %u (max %d), dropping", num, WS_MAX_CLIENTS);
        return;
    }
    switch (type) {
        ...
        case WStype_CONNECTED:
            // Per-IP auth rate limit: refuse the connection outright while a
            // lockout from repeated failed handshakes is active.
            if (isWsAuthLockedOut(num)) {
                Logger::warning("WebPortal", "WS[%u] rejected: auth rate-limited", num);
                m_webSocket->sendTXT(num, "{\"type\":\"error\",\"message\":\"rate limited\"}");
                m_webSocket->disconnect(num);
                break;
            }
            Logger::info("WebPortal", "WS[%u] connected (awaiting auth)", num);
            m_wsAuth[num] = false;
            m_wsLastSeen[num] = millis();
...
void WebPortal::handleWebSocketAuthMessage(uint8_t num, const String& msg) noexcept {
    if (num >= WS_MAX_CLIENTS) return;  // defensive; num is pinned by WEBSOCKETS_SERVER_CLIENT_MAX
    if (m_wsAuth[num]) return;

    JsonDocument doc;
    DeserializationError err = deserializeJson(doc, msg);
    if (err) {
        recordWsAuthFailure(num);
...
void WebPortal::sendToAuthenticatedClients(const String& json) noexcept {
    if (!m_running || m_webSocket == nullptr) return;
    for (size_t i = 0; i < WS_MAX_CLIENTS; ++i) {
        // sendTXT can block up to WEBSOCKETS_TCP_TIMEOUT (5 s per client inside
        // the library's write()); with WS_MAX_CLIENTS slots that approaches the
        // 30 s task watchdog window. Feed the WDT before each blocking send so
        // a batch of slow-but-alive clients cannot trip the watchdog.
        esp_task_wdt_reset();
        if (!m_wsAuth[i]) continue;
        const uint8_t slot = static_cast<uint8_t>(i);
        if (!m_webSocket->clientIsConnected(slot)) {
            // Client went away but no DISCONNECTED event arrived yet; free the
            // slot so a new connection can reuse it instead of accumulating
            // stale entries that would be targeted by every broadcast.
            m_wsAuth[i] = false;
            m_wsLastSeen[i] = 0;
            continue;
        }
        // Reap clients that haven't sent any message/pong for longer than
        // m_clientTimeout (5 minutes). This catches clients that stopped
        // responding but the TCP connection is still open (no FIN/RST).
        if (millis() - m_wsLastSeen[i] >= m_clientTimeout) {
            Logger::warning("WebPortal", "WS[%u] inactive for %lu ms; reaping", i, millis() - m_wsLastSeen[i]);
            m_webSocket->disconnect(slot);
            m_wsAuth[i] = false;
            m_wsLastSeen[i] = 0;
            continue;
        }
        if (!m_webSocket->sendTXT(slot, json.c_str(), json.length())) {
            // The library returned false: the socket is gone or the TCP write
            // failed/timed out. Disconnect the stale slot (idempotent) and mark
            // it unauthenticated so we stop blocking on it in later broadcasts.
            Logger::warning("WebPortal", "WS[%u] sendTXT failed; reaping stale client", i);
            m_webSocket->disconnect(slot);
            m_wsAuth[i] = false;
            m_wsLastSeen[i] = 0;
        }
...
void WebPortal::webSocketBroadcast(const String& json) noexcept {
...
void WebPortal::webSocketBroadcastDashboard() noexcept {
    if (!m_running || m_webSocket == nullptr) return;
    if (m_webSocket->connectedClients() == 0) return;

    // Reuse the member buffer: clear() keeps the already-reserved capacity, so
    // repeated broadcasts avoid a fresh ~2KB heap allocation each time.
    String& json = m_wsDashboardJson;
    json.clear();
    json.reserve(2048);
    json = "{\"type\":\"dashboard\"";
...
void WebPortal::handleApiWifi() noexcept
{
    ...
    else if (m_server.method() == HTTP_POST)
    {
        ...
        // 2. Enter WAITING_TO_CONNECT state (non-blocking 5s wait)
        // 3. After 5s delay, WifiManager will automatically attempt connection
        // This allows Flutter to show "Connecting..." state before radio reboots
        wifiManager.startWaitingToConnect(ssid.c_str(), password.c_str());
        Logger::info("WebPortal", "Wi-Fi credentials saved via API, waiting 5s before connect");
        sendSuccess("Wi-Fi credentials saved. Device will connect in 5 seconds...");
    }
}
...
void WebPortal::handleApiWifiScan() noexcept
{
    ...
    // Non-blocking scan: the request either starts a scan and reports
    // progress, or returns the finished results. The companion app polls
    // this endpoint until it gets "done".
    const WifiManager::ScanState scanState = wifiManager.getScanState();

    if (scanState == WifiManager::ScanState::IDLE) {
        if (!wifiManager.startScan()) {
            sendError("Wi-Fi scan could not be started", 500);
            return;
        }
        sendJson("{\"state\":\"scanning\"}");
        return;
    }

    if (scanState == WifiManager::ScanState::RUNNING) {
        sendJson("{\"state\":\"scanning\"}");
        return;
    }

    if (scanState == WifiManager::ScanState::FAILED) {
        wifiManager.consumeScanResults();
        sendJson("{\"state\":\"failed\"}");
        return;
    }

    // ScanState::DONE — build the results payload, then reset the job so the
    // next request starts a fresh scan.
    const int16_t count = wifiManager.getScanResultCount();
    String json = "{\"state\":\"done\",\"count\":";
    json += String(count);
    json += ",\"networks\":[";
    uint16_t shown = 0;
...
void WebPortal::handleApiMicDiagnose() noexcept
{
    m_requestCounter++;
    if (!isAuthenticatedOrReject()) return;

    MicDiagnostic diag;
    const bool captured = audioManager.runMicDiagnostics(diag);

    char json[MAX_JSON_BUFFER];
    size_t sttUsedBytes = 0, sttCapacityBytes = 0;
    speechToText.getBufferStats(sttUsedBytes, sttCapacityBytes);
    snprintf(json, sizeof(json),
        R"({"ok":%s,"window_ms":%u,"read_calls":%u,"read_errors":%u,"frames":%u,"valid_samples":%u,"min_sample":%ld,"max_sample":%ld,"rms":%.3f,"peak":%.3f,"zero_percent":%.2f,"clip_percent":%.2f,"detected_sample_rate":%u,"stt_buffer_bytes":%u,"stt_buffer_capacity":%u,"stt_buffer_percent":%.1f,"heap_free":%u})",
        captured ? "true" : "false",
        diag.windowMs,
        diag.readCalls,
        diag.readErrors,
        diag.frames,
        diag.validSamples,
        static_cast<long>(diag.minSample),
        static_cast<long>(diag.maxSample),
        static_cast<double>(diag.rms),
        static_cast<double>(diag.peak),
        static_cast<double>(diag.zeroPercent),
        static_cast<double>(diag.clipPercent),
        diag.detectedSampleRate,
        static_cast<unsigned>(sttUsedBytes),
        static_cast<unsigned>(sttCapacityBytes),
        (sttCapacityBytes > 0U) ? (100.0 * static_cast<double>(sttUsedBytes) / static_cast<double>(sttCapacityBytes)) : 0.0,
...
// Storage browser API
//
// Contract with the companion app (SdRepository):
//   GET  /api/storage?path=<dir>          -> {files:[{name,path,dir,size},...]}
//   GET  /api/storage/download?path=...   -> raw file bytes
//   POST /api/storage/upload (multipart)  -> {success:true}
//   POST /api/storage/delete              -> {path:...}
//
// Storage selection: SPIFFS by default (where AURA data actually lives).
// Optional ?storage=sd selects the SD card when it is mounted.
// ============================================================================

static StorageType resolveStorageType(const String& storageArg) noexcept
{
    if (storageArg == "sd") {
        return storageManager.isSDMounted() ? StorageType::SD_CARD : StorageType::SPIFFS;
    }
    return StorageType::SPIFFS;
}

void WebPortal::handleApiStorageList() noexcept
{
    m_requestCounter++;
    if (!isAuthenticatedOrReject()) return;

    String dir = m_server.hasArg("path") ? m_server.arg("path") : String("/");
    if (dir.isEmpty() || dir[0] != '/') dir = "/" + dir;

    const StorageType type = resolveStorageType(m_server.hasArg("storage") ? m_server.arg("storage") : String(""));

    std::vector<FileInfo> entries;
    StorageStatus status = storageManager.listDirectory(dir.c_str(), entries, type);
    if (status != StorageStatus::SUCCESS) {
        sendError("Failed to list directory", 400);
        return;
    }

    String json;
    json.reserve(2048 + entries.size() * 96);
    json += "{\"success\":true,\"path\":\"" + escapeJson(dir) + "\",\"files\":[";

    for (size_t i = 0; i < entries.size(); ++i) {
        const FileInfo& f = entries[i];
        String fullPath;
        if (dir == "/") {
            fullPath = "/" + f.name;
        } else {
        ...
        json += "\"path\":\"" + escapeJson(fullPath) + "\",";
        json += "\"dir\":" + String(f.isDirectory ? "true" : "false") + ",";
        json += "\"size\":" + String(static_cast<unsigned long long>(f.size)) + "}";
    }
...
void WebPortal::handleApiStorageDownload() noexcept
{
    m_requestCounter++;
    if (!isAuthenticatedOrReject()) return;

    if (!m_server.hasArg("path")) {
        sendError("Missing path", 400);
        return;
    }
    const String path = m_server.arg("path");
    const StorageType type = resolveStorageType(m_server.hasArg("storage") ? m_server.arg("storage") : String(""));

    String content;
    StorageStatus status = storageManager.readFile(path.c_str(), content, type);
    if (status != StorageStatus::SUCCESS) {
        sendError("File not found", 404);
        return;
    }
    m_server.send(200, "application/octet-stream", content);
}

void WebPortal::handleApiStorageDelete() noexcept
{
    m_requestCounter++;
    if (!isAuthenticatedOrReject()) return;
    ...
    const StorageType type = resolveStorageType(doc["storage"] | "");

    // Try file deletion first, then directory removal (SPIFFS reports NOT_FOUND
    // for non-existent paths; a directory path returns an IO error on remove).
    StorageStatus status = storageManager.deleteFile(path, type);
    if (status == StorageStatus::ERROR_IO) {
        status = storageManager.removeDirectory(path, type);
    }
    if (status != StorageStatus::SUCCESS) {
    ...
    sendSuccess("Deleted " + String(path));
}

void WebPortal::handleApiStorageUploadChunk() noexcept
{
    // Auth must gate the buffering itself: the buffer (up to
    // kStorageUploadMaxBytes) is accumulated here, before the final handler's
    // isAuthenticatedOrReject(). An unauthenticated client must not be able to
    // force that memory to be filled per request.
    if (!isAuthenticated()) {
        if (m_server.upload().status == UPLOAD_FILE_START) {
            m_storageUploadActive = false;
            m_storageUploadBuffer = "";
        }
        return;
    }
    ...
    if (upload.status == UPLOAD_FILE_START)
    {
        m_storageUploadName = upload.filename;
        m_storageUploadBuffer = "";
        m_storageUploadActive = true;
        m_storageUploadDone = false;
    }
    else if (upload.status == UPLOAD_FILE_WRITE)
    {
        if (m_storageUploadActive)
        {
            if (m_storageUploadBuffer.length() + upload.currentSize > kStorageUploadMaxBytes)
            {
                m_storageUploadActive = false;
                return;
            }
            m_storageUploadBuffer.concat(reinterpret_cast<const char*>(upload.buf), upload.currentSize);
        }
    }
    else if (upload.status == UPLOAD_FILE_END)
    {
        m_storageUploadDone = m_storageUploadActive;
        m_storageUploadActive = false;
    }
    else if (upload.status == UPLOAD_FILE_ABORTED)
    {
...
void WebPortal::handleApiStorageUpload() noexcept
{
    m_requestCounter++;
    if (!isAuthenticatedOrReject()) return;

    if (!m_storageUploadDone || m_storageUploadName.isEmpty())
    {
        m_storageUploadDone = false;
        sendError("Upload failed or aborted", 400);
        return;
    }

    const StorageType type = resolveStorageType(m_server.hasArg("storage") ? m_server.arg("storage") : String(""));

    // Store to the upload target directory, or root when no directory given.
    String dir = m_server.hasArg("path") ? m_server.arg("path") : String("/");
    if (dir.isEmpty() || dir[0] != '/') dir = "/" + dir;
    if (dir != "/" && !dir.endsWith("/")) dir += "/";
    const String targetPath = (dir == "/" ? "/" : dir) + m_storageUploadName;

    StorageStatus status = storageManager.writeFile(
        targetPath.c_str(),
        reinterpret_cast<const uint8_t*>(m_storageUploadBuffer.c_str()),
        m_storageUploadBuffer.length(),
        type);
    ...
    if (status != StorageStatus::SUCCESS) {
        sendError("Failed to write file", 500);
        return;
    }

    sendSuccess("File uploaded to " + targetPath);
}
...
String WebPortal::wsAuthKey(uint8_t num) const noexcept {
    // Key the tracker by the client's IP so a device cycling WS slots cannot
    // bypass the limit. Fall back to the slot index if the library reports a
    // zero IP (e.g. mid-handshake) so the key is still bounded.
    const IPAddress ip = (m_webSocket != nullptr) ? m_webSocket->remoteIP(num) : IPAddress(0, 0, 0, 0);
    if (ip == IPAddress(0, 0, 0, 0)) {
        return String("slot") + String(num);
    }
    return ip.toString();
}

void WebPortal::recordWsAuthFailure(uint8_t num) noexcept {
    if (num >= WS_MAX_CLIENTS) return;
    const String key = wsAuthKey(num);
    LoginTracker& tracker = m_wsAuthTrackers[key];
    tracker.attempts++;
    if (tracker.attempts >= kWsMaxAuthAttempts) {
        tracker.lockoutUntil = millis() + kWsAuthLockoutDurationMs;
        tracker.attempts = 0;
        Logger::warning("WebPortal", "WS auth locked out for %lu ms for %s",
            kWsAuthLockoutDurationMs, key.c_str());
    }

    // Bound memory usage: drop stale trackers once the map grows too large.
    if (m_wsAuthTrackers.size() > kWsMaxTrackedIps) {
        for (auto it = m_wsAuthTrackers.begin(); it != m_wsAuthTrackers.end();) {
            if (it->second.lockoutUntil == 0 || millis() > it->second.lockoutUntil) {
                it = m_wsAuthTrackers.erase(it);
            } else {
                ++it;
            }
        }
...
bool WebPortal::isWsAuthLockedOut(uint8_t num) noexcept {
    if (num >= WS_MAX_CLIENTS) return false;
    const String key = wsAuthKey(num);
    auto it = m_wsAuthTrackers.find(key);
    if (it == m_wsAuthTrackers.end()) return false;
    return millis() < it->second.lockoutUntil;
}

void WebPortal::resetWsAuthTracker(uint8_t num) noexcept {
    if (num >= WS_MAX_CLIENTS) return;
    m_wsAuthTrackers.erase(wsAuthKey(num));
}
...
void WebPortal::handleApiEspNowPair() noexcept
{
    ...
    sendSuccess(espNowManager.pairNodeLocked(mac, nodeType) ? "Node paired" : "Failed to pair node");
}
...
static void restartTask(void* pvParameters)
{
    ...
    systemManager.requestRestart("web_restart", true);
    vTaskDelete(nullptr);
}
...
static void factoryResetTask(void* pvParameters)
{
    ...
    // Clear all device settings too, so the next boot starts from factory
    // defaults rather than a half-reset configuration. Bounded NVS operation.
    ...
    systemManager.requestRestart("web_factory_reset", true);
    vTaskDelete(nullptr);
}
...
static void otaRestartTask(void* pvParameters)
{
    ...
    systemManager.requestRestart("web_ota_apply", false);
    vTaskDelete(nullptr);
}
