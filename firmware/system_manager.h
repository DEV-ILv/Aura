...
public:
    ...
    /**
     * @brief Restart the device through the single restart choke point.
     * @param reason Short human-readable reason, recorded to NVS for
     *               next-boot diagnostics (never secrets)
     * @param flushState Persist settings and uptime before resetting
     */
    void requestRestart(const char* reason, bool flushState) noexcept;
...
private:
    ...
    void monitorSubsystems() noexcept;
