...
enum class WifiState : uint8_t {
  ...
  WAITING_TO_CONNECT,  ///< Credentials received, waiting before connection attempt
  ERROR                ///< Error state
};
...
public:
   ...
   */
  [[nodiscard]] bool startWaitingToConnect(const char* ssid, const char* password) noexcept;
   ...
   * @deprecated Use startScan() + getScanState() for a non-blocking scan.
   */
  [[nodiscard]] int16_t scanNetworks() noexcept;

  /**
   * @brief State of the asynchronous Wi-Fi scan job.
   */
  enum class ScanState : uint8_t {
    IDLE,     ///< No scan in progress
    RUNNING,  ///< Scan in flight (non-blocking; poll getScanState())
    DONE,     ///< Scan finished; results ready via getNetworkInfo()
    FAILED    ///< Scan failed or was not triggered
  };

  /**
   * @brief Start an asynchronous Wi-Fi scan.
   *
   * Kicks off the scan on the driver and returns immediately so the main
   * loop, HTTP server, WebSocket server and setup AP stay responsive. Use
   * getScanState() to poll for completion. A second scan is refused while
   * one is already running (scans are serialized).
   * @return true if the scan was started, false if refused (already running
   *         or the driver rejected the request).
   */
  [[nodiscard]] bool startScan() noexcept;

  /**
   * @brief Get the current state of the asynchronous scan job.
   */
  [[nodiscard]] ScanState getScanState() const noexcept;

  /**
   * @brief Get the number of networks found by the last completed scan.
   * @note Only meaningful when getScanState() == ScanState::DONE.
   */
  [[nodiscard]] int16_t getScanResultCount() const noexcept;

  /**
   * @brief Consume and release the last scan results.
   *
   * Frees the driver scan buffer and resets the job to IDLE so a later call
   * to startScan() begins a fresh scan. Safe to call in any state.
   */
  void consumeScanResults() noexcept;
...
private:
  ...
  void pollScan() noexcept;
  String getAccessPointPassword() const noexcept;
  ...
  uint32_t m_waitConnectTimer;        ///< Timer for WAITING_TO_CONNECT delay
  ...
  bool m_provisioning;                ///< Provisioning in progress (awaiting 5s wait)
  ...
  uint32_t m_reconnectCount;          ///< Total connection attempts since boot
  ...
  ScanState m_scanState;              ///< Async scan job state
  int16_t m_scanResultCount;          ///< Networks found by the last scan
  uint32_t m_scanStartedMs;           ///< Timestamp when the scan started
  uint32_t m_scanDurationMs;          ///< Duration of the last completed scan
};
