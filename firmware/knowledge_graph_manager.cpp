...
bool KnowledgeGraphManager::save() noexcept {
    String json;
    // Content-aware reserve: sum node/edge string lengths + fixed overhead.
    size_t totalLen = 2; // "["
    for (const auto& n : m_nodes) {
        totalLen += n.id.length() + n.name.length() + n.value.length() + n.tags.length();
        totalLen += 120; // fixed JSON overhead per node
    }
    for (const auto& e : m_edges) {
        totalLen += e.id.length() + e.sourceId.length() + e.targetId.length() + e.relationship.length();
        totalLen += 140; // fixed JSON overhead per edge
    }
    totalLen += 64; // outer {"nodes":[...],"edges":[...]}
