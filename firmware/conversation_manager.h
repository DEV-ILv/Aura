...
struct ConversationResult {
    ...
    unsigned long sttLatencyMs;    ///< STT recognition latency
    unsigned long llmLatencyMs;    ///< LLM response latency
    unsigned long ttsLatencyMs;    ///< TTS synthesis+playback latency
    unsigned long timestamp;   ///< Completion timestamp
    ConversationError error;   ///< Error code if failed

    ConversationResult() noexcept
        : userText(""), assistantText(""), latencyMs(0), sttLatencyMs(0), llmLatencyMs(0),
          ttsLatencyMs(0), timestamp(0), error(ConversationError::NONE) {}
...
void handleRestartHold() noexcept;
    ...
    // Barge-in debounce: require several consecutive above-threshold mic probes
    // while SPEAKING before interrupting, so a brief click/ambient blip does not
    // cut the assistant off.
    unsigned long m_bargeInStartMs{0};   ///< Time the above-threshold streak began
    unsigned long m_bargeInLastCheckMs{0}; ///< Last probe timestamp
    static constexpr unsigned long kBargeInRequireMs = 120UL;
    ...
    static constexpr unsigned long kErrorRecoveryMs = 10000UL;
