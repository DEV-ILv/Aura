...
bool AnalyticsManager::save() noexcept {
    ...
    String json;
    // Content-aware reserve: sum string lengths + fixed overhead per record.
    // This avoids the exact-fit reallocation churn seen with fixed multipliers.
    size_t totalLen = 2; // "["
    for (const auto& r : m_records) {
        totalLen += r.category.length() + r.metric.length() + r.unit.length() + r.tags.length();
        totalLen += 100; // fixed JSON overhead per record
    }
    totalLen += 64; // outer {"records":[ ... ]}

    // Heap floor guard: skip save if not enough heap to safely build JSON.
    // This prevents OOM-truncated file writes that would corrupt persistent state.
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping save: low heap (%u < %u + %u)",
            ESP.getFreeHeap(), totalLen, kHeapSlack);
        return false; // m_dirty remains true; will retry on next save interval
    }
