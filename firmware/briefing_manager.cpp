...
bool BriefingManager::saveBriefings() noexcept {
    String json;
    // Content-aware reserve: sum content lengths + fixed overhead per record.
    // Briefing content can be up to BRIEFING_MAX_LENGTH (2048) per record,
    // so a fixed multiplier of 160 was severely under-reserving (~13x for full records).
    size_t totalLen = 2; // "["
    for (const auto& r : m_records) {
        totalLen += r.id.length() + r.type.length() + r.content.length();
        totalLen += 80; // fixed JSON overhead per record
    }
    totalLen += 48; // outer {"version":...,"records":[ ... ]}

    // Heap floor guard.
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping saveBriefings: low heap (%u < %u + %u)",
...
bool BriefingManager::saveSummaries() noexcept {
    if (!m_initialized) return false;
    String json;
    // Content-aware reserve: sum content lengths + fixed overhead per summary.
    size_t totalLen = 1; // "["
    for (const auto& s : m_summaries) {
        totalLen += s.id.length() + s.date.length() + s.content.length();
        totalLen += 120; // fixed JSON overhead per summary
    }
    totalLen += 2; // "]"
    ...
    const size_t kHeapSlack = 8192;
    if (ESP.getFreeHeap() < totalLen + kHeapSlack) {
        Logger::warning(kLogCategory, "Skipping saveSummaries: low heap (%u < %u + %u)",
            ESP.getFreeHeap(), totalLen, kHeapSlack);
        return false;
    }
