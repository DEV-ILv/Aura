...
> the current state (Sarvam AI placeholders — see
...
**Target:** ESP32-WROOM-32 (38-pin) · Arduino-ESP32 core 3.3.11
...
- `bool initialize() noexcept` — full boot orchestration.
- `void run() noexcept` / `void update() noexcept` — main-loop tick; calls
  `updateModules()` + `checkHealth()`.
- `void shutdown() noexcept` — save-all + graceful stop, reverse-order.
- `void restart() noexcept` — optional crash log, then `ESP.restart()`.
- `void factoryReset() noexcept` — format SPIFFS, clear
...
- `SarvamSpeechToText`: `initialize()`, `update()`, `startRecognition()`,
  `stopRecognition()`, `processAudio()`, `getResult()`, `isListening()` —
...
- `SarvamTextToSpeech`: `initialize()`, `update()`, `speak(text)`, `stop()`,
  `isSpeaking()`, `isInitialized()` — placeholder.
...
- `SecurityManager`: `Authenticate(token)`, `Deauthenticate()`,
  ...
  `RevokeSession(...)`, `Encrypt/Decrypt` (return plaintext — stub),
...
`updateModules()` — a hardcoded sequential list of ~40 `.update()` calls
...
(`initializeModules()`). Hard-fail modules (return `false` → boot aborts →
...
5. `WifiManager` → Storage; starts AP if no credentials
6. `UiFramework` → Display
7. `AudioManager` (I2S) → `SoundManager`, `AudioAssetManager`
8. `WebPortal` → WiFi
9. STT → WiFi; `GeminiClient` → WiFi; TTS → WiFi + Audio
10. `ConversationManager` → STT, Gemini, TTS
11. `ReminderManager`, `OtaManager`
12. `SettingsManager`, `PluginManager` (→SD), `SkillManager`,
...
- Gemini (`GEMINI_URL`) — `GeminiClient`.
- Sarvam AI (`SARVAM_BASE_URL`) — `SarvamSpeechToText` / `SarvamTextToSpeech`
...
(`memoryManager.semanticSearch`) — note: `SemanticSearchManager` is **not**
...
`studyManager.getDueSubjects()`. Response path: Gemini → `FunctionRouter`
tool calls (goals, planner, etc.) → TTS playback + OLED rendering.
...
memory consolidation → `knowledgeGraphManager.autoLink()` →
...
1. `setup()`: `Serial.begin(115200)` → `Logger::initialize()` →
   `logBootDiagnostics()` → `initializeWatchdog()` (30 s task WDT) →
   `systemManager.initialize()`.
2. `SystemManager::initialize()`:
   - Reads NVS boot-loop counter pre-init; sets `m_safeMode` if ≥ 3.
   - `changeState(INITIALIZING)`; fills `SystemInfo`.
   - `initializeModules()` — sequential module init (see §5.2); on hard
     failure → `rollbackInitialization()` + `ERROR`/`INIT_FAILED` +
...
3. `loop()`: `systemManager.run()` → `update()` → `updateModules()` +
...
- **State machine:** `SystemState` — `BOOTING → INITIALIZING → READY ↔
  BUSY/LOW_POWER/UPDATING → ERROR/SHUTDOWN` — enforced by a static
...
- **Reminder annunciation:** state machine `IDLE → WAITING → TRIGGERED →
  SPEAKING → COMPLETED/ERROR`; `isBusy()` gates system sleep.
...
- **WiFi:** single radio owner — `WifiManager` (see
...
- **Wiring gaps (behavioral impact):**
  - `ServiceManager::Update()` never runs registered services (no
    ...
    (some expose functions driven elsewhere — e.g. `commandPalette` is called
  ...
  - `SmartSearch.search()/quickSearch()` — zero callers; feature unreachable.
  - `TimelineManager.addEntry()` — zero callers; every timeline read is empty.
  - `AnalyticsManager.record()` — zero callers; study minutes/trend always 0.
  - `ReminderManager.addReminder()` — no web-portal endpoint; reminders
    cannot be created from the UI.
  - `ContextManager` setters — only 4 of 16 are ever called (mood,
    conversation count, last conversation, conversation topic — all from
    `ConversationManager`); prompt context is largely default/static.
  - `DocumentManager.reindexAll()/linkToGraph()/summarize()` — zero callers;
    documents never enter the knowledge graph.
  - `LearningManager.observe()` — portal-only; no organic observation sources.
...
| --- | --- | --- |
| Voice assistant (STT→Gemini→TTS) | Wake via touch (GPIO13) / command | Partially wired (cloud endpoints not set; Sarvam placeholders) |
| Offline assistant responses | `OfflineResponseGenerator` / `LocalAIEngine` (26 intents) | Complete |
| Web configuration & dashboard | `WebPortal` (REST + WebSocket, token auth) | Complete — largest live consumer |
| OTA firmware update (signed) | `OtaManager` + web portal | Complete (ECDSA P-256 + streaming SHA-256 verify) |
| Reminders | `ReminderManager` | Broken — cannot be created |
...
| Automation / workflows / scenes / command palette | `AutomationManager`, `WorkflowEngine`, `SceneEngine`, `CommandPalette` | Partially wired (see §8) |
...
See [WIRING.md](WIRING.md) for the full verified pinout. Summary (pins from
`config.h`):
...
| Flash / NVS | internal | — | Storage, `Preferences`, crash counter |

## 11. Configuration

- **`config.h`** — hardware pins, `aura::identity` (`kProjectName` "AURA AI
...
- **`version.h`** — single-source versioning: `kMajor/kMinor/kPatch` (1.0.0),
  `kMark`=3 (MARK III), `kCodename` "Phoenix", `kChannel` "Development",
  `kOsName` "AURA OS", compile-time build date/time.
- **`secrets.h`** — `GEMINI_API_KEY`, `SARVAM_API_KEY`, `AP_SSID`,
...
- **`firmware_keys.h`** — ECDSA P-256 public key for OTA signature
  verification.
- **Runtime settings** — `SettingsManager` (user prefs), WiFi creds in NVS
  `aura_wifi`, personality profile selection, plugin marketplace file
  `/plugin_marketplace.json`.

## 12. Error Recovery

- **Watchdog:** 30 s task WDT on the loop task; stuck module → device reset.
- **Boot-loop protection:** NVS counter incremented on abnormal restart;
  ≥3 → Safe Mode; cleared on successful boot
...
- **Rollback:** failed `initializeModules()` triggers
  `rollbackInitialization()` — reverse-order save + graceful stop per
  initialized module index.
- **ResilienceManager:** 12 `FailureType` recovery plans (WiFi/SD/Renderer/AI)
  with attempts/cooldowns — **dead code**: only `Initialize()` is called;
...
```
                    Aura_programs.ino (setup / loop)
                              │
                    ┌─────────▼──────────┐
                    │    SystemManager   │  state machine + health
                    │  (single loop)     │
                    └───┬──────┬──────┬──┘
                        │      │      │
        updateModules() │      │      │ checkHealth()
   (sequential poll,    │      │      └─ memory/WiFi/OTA/reminder/conversation monitors
    ~40 managers)       │      │
                        │      └── ServiceManager (registered services — NOT started)
                        │                 ├─ CapabilityManager, PlatformAbstraction,
                        │                 ├─ TaskScheduler, AiPipeline, CommandPalette,
                        │                 ├─ SceneEngine, WorkflowEngine, LogManager,
                        │                 ├─ SecurityManager, ResilienceManager, DiagnosticSystem
                        │
        Core: StorageManager · MemoryManager · EventBus · Logger
        Voice: AudioManager(I2S) · SarvamSTT · GeminiClient · SarvamTTS · TinyAI · Offline
        UI:    DisplayManager · UiFramework · ScreenManager · WidgetEngine · LedRing
        Net:   WifiManager · WebPortal · OtaManager · EspNowManager · DeviceMesh · Companion
        Prod:  ExecutiveAssistant · Briefing · Reminder · Study · Goal · Habit · Planner ·
               Reflection · Timeline · Document · Workspace · Context · KG · SemanticSearch ·
               SmartSearch · Decision · Learning · Prediction · Analytics
        Rel:   CrashManager · ResilienceManager · Diagnostics · Performance · HealthMonitor
        Ext:   Plugin · Skill · Automation · WorkflowEngine · SceneEngine · CommandPalette
```
...
**Wiring verdict legend:** COMPLETE (driven end-to-end) · PARTIAL (initialized
+ partially consumed) · DEAD/STUB (no external callers).
...
- **Total:** ~160 source files — ~84 headers, ~75 `.cpp`, 1 `.ino`.
- **Total lines:** ~42K (headers ~12K · sources ~29.5K · sketch ~134).
...
- **Flash:** ~1,938,167 / 3,145,728 bytes (61%) — *default partition scheme
...
STUB/DEAD in §14 should be treated as scaffolding or reserved functionality
