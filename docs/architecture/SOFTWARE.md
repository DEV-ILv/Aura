...
- **Partition scheme:** Huge APP (3 MB No OTA / 1 MB SPIFFS) â€” the default
  scheme does not fit.
...
- **Configuration:** `config.h` (pins, identity, AI endpoint URLs, timing
  constants), `secrets.h` (API keys / credentials â€” **git-ignored**, copy the
...
- **Remote (cloud) mode:** falls back to **Supabase** when the device is out
  of range â€” email/password sign-in, cloud device registry. RLS protects all
...
- **Device Control centre:** display, LED ring, speaker, microphone, and
  ...
  See [../development/companion-v2-device-control.md](../development/companion-v2-device-control.md).

Integration specs:
- [../development/companion-output.md](../development/companion-output.md) â€” assistant-response WebSocket
  message format.
- `aura_companion/docs/supabase.md` â€” Supabase schema, RLS, auth flow,
  local-vs-remote architecture.

Build: `flutter pub get` â†’ `flutter analyze` â†’ `flutter test` â†’
...
| Speech-to-Text (firmware) | **Sarvam AI** (`sarvam_stt.*`) | Placeholder â€” not implemented |
| Text-to-Speech (firmware) | **Sarvam AI** (`sarvam_tts.*`) | Placeholder â€” not implemented |
