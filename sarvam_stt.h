...
// buffers 16 kHz mono PCM in a static WAV/multipart buffer. A lightweight VAD
// on the captured samples ends an utterance after a quiet period (or the cap).
// On end, the caller (ConversationManager) sees isBusy() go false; the final
// transcript is reported through getResult().
//
// Network I/O is async via SarvamClient (https + retry). No caller-facing
// interface changed; speech_provider.h contract is fully honored.
// ============================================================================
...
private:
    ...
    void assembleMultipart() noexcept;
    ...
    uint8_t* m_upBuf{nullptr};
    size_t m_upCap{0};

    static constexpr size_t kPcmOffset   = 512;                // reserve for head+wav
    static constexpr size_t kCaptureCap  = SARVAM_MAX_AUDIO_BYTES; // bytes of PCM
};
